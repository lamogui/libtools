...
#include <libtools/decoders/mididecoder.hpp>
...
MidiHeader::MidiHeader(uint16_t format, uint16_t tracks, uint16_t tick_per_beat) :
  _beat(false),
...
MidiHeader::MidiHeader(uint16_t format, uint16_t tracks, uint8_t frame, uint8_t ticks) :
  _beat(true),
...
void MidiHeader::print_infos() const
{
  printf("MidiHeader 0x%X infos: \n", this);
  printf("      format: %u \n", _format);
  printf("      tracks: %u \n", _tracks);
...
bool MidiHeader::read_from_buffer(const unsigned char* buffer, unsigned int size )
{
  unsigned int g=0;
  if (!buffer) return false;
  if (size < MidiHeader::size) return false;
  ...
  if (format !=1 && format !=0){
    printf("MidiHeader error: Format %u unsupported in reading yet\n",format);
    return false;
  }
  ...
  if (division & 0x8000) {
    uint8_t frame=(division >> 8) & 0x7F, ticks=(division & 0xFF);
    if (! frame*ticks)
    {
      printf("MidiHeader error: invalid fps time division\n");
      return false;
    }
    _gain=(float) frame*ticks / (float) Signal::refreshRate;
    _beat=false;
    ...
    if (!division){
      printf("MidiHeader error: invalid beat time division\n");
      return false;
    }
...
bool MidiHeader::write_to_buffer(unsigned char* buffer, unsigned int size ) const {
  if (!buffer) return false;
  if (size < MidiHeader::size) return false;
  ...
  *buffer++=6;
  ...
  if (_format!=1) {
    printf("MidiHeader error: Format %u unsupported in writing yet\n",_format);
    return false;
  }
  else {
    *buffer++=0;
...
bool MidiHeader::write_to_file(FILE* file) const
{
  unsigned char buffer[MidiHeader::size];
  if (write_to_buffer(buffer,MidiHeader::size))
  {
    if (MidiHeader::size==fwrite((void*)buffer,1,MidiHeader::size,file))
    {
      return true;
    }
    printf("MidiHeader error: fwrite error !\n");
  }
  return false;
}
  ...
  if (s > min_size)
  {
    ...
    if (s > target_size)
    {
      while (g < target_size)
      {
        ...
        //Read type
        uint8_t type=buffer[g++];
        if (type==0xFF) { //Meta Event 
          if (g >= target_size) break;
          uint8_t meta_type=buffer[g++];
...
Midi_track::Midi_track(MidiHeader& head) :
_head(&head),
_chunk(0),
_alloc_size(256),
_chunk_size(0)
...
void Midi_track::push_varlength(uint32_t var) {
  uint32_t var_tp=var;
  uint32_t bit=0;
  uint8_t part;
  ...
  for (uint32_t i=0; i<(unsigned)part+1; i++) {
    if (i==part) {
...
void Midi_track::push_midi_event(uint32_t midi_delta, uint8_t type, uint8_t chan, uint8_t p1, uint8_t p2)
{
...
void Midi_track::push_midi_event(uint32_t midi_delta, uint8_t type, uint8_t chan, uint8_t p1)
{
  push_varlength(midi_delta);
  check_alloc(2);
  _chunk[_chunk_size++] = (type & 0xF) << 4 | (chan & 0xF);
...
Midi_event::Midi_event(MidiVar d,uint8_t type):
...
Midi_event::Midi_event(uint8_t type, uint8_t channel, uint16_t _p1 ) :
...
Midi_event::Midi_event(uint8_t type, uint8_t channel, uint16_t _p1 , uint16_t _p2) :
Midi_abstractevent((type << 4) | channel),
p1(_p1),
p2(_p2)
{
...
Midi_metaevent::Midi_metaevent(MidiVar d,uint8_t meta_type) :
...
Midi_metaevent* Midi_metaevent::create_from_buffer(const unsigned char* buffer, 
  ...
  if (buffer_size >= offset + 1) {
    uint8_t meta_type = buffer[offset++];
    switch (meta_type)
    {
      default: 
